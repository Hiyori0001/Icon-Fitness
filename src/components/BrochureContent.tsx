    ...
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {machines.map((machine) => (
          <Card key={machine.id} className="flex flex-col overflow-hidden shadow-lg border border-gray-200">
            ...
            <CardContent className="p-4 flex-grow flex flex-col">
              <CardTitle className="text-xl font-bold text-primary mb-2">{machine.name}</CardTitle>
              <CardDescription className="text-gray-700 text-sm mb-3 flex-grow">{machine.description}</CardDescription>
              {includePrice && (
                <p className="text-lg font-bold text-secondary-foreground mt-auto">{formatCurrencyINR(machine.price)}</p>
              )}
            </CardContent>
          </Card>
        ))}
      ...
      {(includePrice || customPrice) && machines.length > 0 && (
        <>
          <Separator className="my-8 bg-gray-300" />
          <div className="mt-10 p-6 bg-card rounded-lg shadow-xl border border-gray-200">
